...
- On `fix/househelp-main-menu-flow`, a househelp hears and browses active assigned dishes one at a time, chooses or resumes one, can review prior cooking steps without rewinding saved progress, and can explicitly return to the menu from an active or completed task. `Done` also returns automatically through a bounded fallback when confirmation speech stalls.
