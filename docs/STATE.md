...
- On `fix/househelp-main-menu-flow`, a househelp hears and browses active assigned dishes one at a time, chooses or resumes one, and returns to the menu after `Done`; the existing English/Hindi ingredient, step, timer, help, issue, retry, and resume behavior remains intact.
